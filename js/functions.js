function getItems(classe, start, count) {
	$.getJSON('json.php?get=item&classe=' + classe + '&start=' + start + '&count=' + count, function(data) {
		...
		if (start > 0) pages.find('#pages').append('<a class="back" href="javascript:void(0);" onClick="getItems(' + classe + ', ' + (start - count) + ', ' + count +');"><< Anterior</a>');
		pages.find('#pages').append('<a class="next" href="javascript:void(0);" onClick="getItems(' + classe + ', ' + (start + count) + ', ' + count + ');">Próxima >></a>');
